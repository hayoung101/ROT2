...
- `move_robot(robot, x, y, rot)` — 이동 + 회전. 변형과 분리. 이동 거리 비례 애니메이션 duration. **실행 직후 자동 검증(transform과 동일)**
