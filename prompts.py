...
INTENT_PROMPT = (
    ...
    "【confirmation_message】 파악한 내용을 사용자에게 확인하는 한 문장. 형식을 고정하라: '(인원·구성과 활동 요약)하시는군요, (공간)에 (준비할 가구·역할과 개수)를 준비해드릴게요.' 항상 이 순서(상황 요약 → 어디에 무엇을 몇 개 준비할지)를 지키고, 패널 각도·좌표 같은 로봇 내부 용어는 쓰지 말고 사람이 쓰는 가구 이름과 개수로 표현하라. 입력의 새 발화에 '(확인 답변)'이 포함돼 있다면 직전에 되묻기를 거쳐 답을 받은 것이니 '네,'로 시작해 답변을 반영했음을 드러내라. 예: (되묻기 없이) '아이와 함께 공작 놀이를 하시는군요, 거실에 넓은 작업 테이블을 준비해드릴게요.' / (되묻기 후) '네, 성인 두 분이 함께 식사하시는군요. 부엌에 식탁 옆 의자 2개를 준비해드릴게요.'"
)
...
INTENT_SCHEMA = {
    ...
    "properties": {
        ...
        "confirmation_message": {
            "type": "string",
            "description": "HITL-1(human-in-the-loop)용 의도 확인 발화 한 문장. 형식 고정: '(인원·구성과 활동 요약)하시는군요, (공간)에 (준비할 가구·역할과 개수)를 준비해드릴게요.' 로봇 내부 용어(패널 각도·좌표) 금지, 가구 이름과 개수로 표현. 되묻기(clarification) 답변을 반영하는 경우 '네,'로 시작",
        },
    },
    "required": [
...
AGENT_PROMPT = (
    ...
    "【자리 선택 — find_placement】 좌표를 직접 지어내지 말고 find_placement에 맡겨라. near에 가구 id를 주면 그 가구에 인접한 후보를, near를 비우면 방 전체의 가용 공간 조사 결과를 준다. 각 후보에는 tag와 주변 여유 clearance(cm), 앵커를 바라보는 rot_suggest가 붙는다 — tag 예: 'table_1_front'(가구 앞), 'table_1_side', 'open_area'(가장 넓은 빈 공간), 'wall_north'(벽가). 활동의 성격과 tag를 맞춰 골라라: 활동이 특정 가구와 연관되면 그 가구 앞(식사→식탁 앞), 넓은 공간이 필요한 활동(놀이·공작)은 clearance 큰 open_area, 집중이 필요한 활동은 벽가. 패널이 기능하는 방향(상판 확장, 등받이, 수납 입구)이 앵커 가구나 활동 지점을 향하도록 rot_suggest를 참고해 rot를 정하라. 앵커를 향해 열 패널은 후보의 panel_toward_anchor(left/right)를 그대로 쓰라 — rot과 패널 좌우의 관계를 직접 계산하지 마라(자주 틀린다). panel_toward_anchor는 그 후보의 rot_suggest를 그대로 채택했을 때만 유효하다. 이때 앵커 반대쪽(먼 쪽) 패널은 자동으로 그 반대 값이다 — panel_toward_anchor가 right면 반대쪽은 left. 등받이처럼 앵커 반대쪽으로 열 패널은 이 반대 값을 쓰면 된다. rot을 rot_suggest와 다르게 바꾸면 이 값은 무효가 되므로, rot은 rot_suggest를 유지하고 '어느 패널을 여는가'로 형태를 해결하라."
