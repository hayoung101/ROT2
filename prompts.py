...
FORM_PROMPT = (
    ...
    "추론 예시(규칙 아님, 원리): '다리 아파, 앉고 싶어'(1명) → 좌면이 40cm로 얕다 → 한 패널 180°를 등받이로, 다른 패널 45°를 좌면 끝의 다리받침 경사로 → panels [180, 45]. 예시에 없는 요청도 몸·물건과의 만남에서 각도를 유도하라."
    ...
    "facing=정면으로 마주 보고 쓰는 가구(식탁 앞 의자, 책상, 독서대)만 — anchor를 마주 본다. "
    "alongside=곁에서 거드는 가구(사이드 테이블·협탁·트레이)는 앉은 자리 '옆'에 짝 가구와 같은 방향으로 나란히 — anchor 곁에 나란히 선다. "
    "pair=두 대를 연결해 하나의 강체로(마주보기 책장, ∧자 지붕). "
    ...
    "관계 맺기는 기본값이 아니다 — 관계는 활동이 요구할 때만 만든다. 다만 '중복하지 말되 반드시 기여하라': 기존 가구가 활동의 중심(책장·식탁·소파)을 이미 제공하면 로봇은 같은 가구가 되려 말고 곁에서 짝이 되는 보완 역할(책장 옆 독서 의자, 소파 앞 티테이블)을 맡아라. 활동 지원 요청에 로봇이 아무것도 안 해서는 안 된다."
    ...
    "【connection】 두 대를 물리적으로 연결할 때만 채운다(mode:pair와 짝). 나란히(side)는 rot 동일·본체 맞대기, 마주보기(face)는 rot 차 180°·패널 맞대기다. 두 로봇을 서로 붙여 두려면 connection을 써라 — 하나의 가구를 만드는 경우(마주보기 책장·∧자 지붕)뿐 아니라, 작업면과 수납처럼 손이 닿아야 하는 짝도 여기 해당한다. 정확한 좌표·각도는 코드가 계산하니 너는 '누구를 누구에게 어떤 방식으로 붙일지'만 선언하라. 맞댈 패널 각도는 네가 정한 panels에서 코드가 유도하므로 따로 적지 않는다. 연결이 없으면 null. "
...
PLACE_PROMPT = (
    ...
    "【패널의 앞면 방향을 읽는 법】 패널의 앞면 방향은 각도에 따라 다르다. 45°는 패널 바깥쪽, 90°는 위쪽, 135°와 180°는 본체 쪽을 향한다. 따라서 등받이(180°)나 독서대(135°)의 사용면을 앵커 쪽 사람에게 보이려면 그 패널은 보통 앵커 반대쪽에 있어야 한다. panel_faces가 각 rot에서 어느 패널이 어디를 향하는지 알려주니 그 값으로 판단하라. "
    "panel_faces.toward는 기준 사물(ref — 앵커가 있으면 앵커, 없으면 가장 가까운 사물) 대비 상대 방향이다: panel_on_ref_side=기준 쪽에 달린 패널(손이 닿는 쪽), front_faces_ref={right/left 각각의 앞면이 기준을 향하는가}. front_faces_ref는 그 후보의 확정된 패널 각도로 코드가 이미 계산해 둔 사실이다 — 네가 각도별로 따져 볼 필요 없이 그대로 읽어라. 등받이·독서대의 사용면을 기준 쪽 사람에게 보여야 한다면 그 패널 쪽 front_faces_ref가 true인 후보를 골라라. 같은 자리라도 뒤집힌 배향이 후보에 짝으로 들어 있으니 이 값으로 갈라진다. "
    ...
    "단, 이 완화 사실은 **네가 고를 때 쓰는 정보이지 사용자에게 할 말이 아니다** — message에는 절대 담지 마라. 사용자는 원안을 본 적이 없다(원안은 내부 정보다). '원래는 소파 앞에 놓으려 했는데 자리가 없어서'라고 하면 사용자가 본 적 없는 계획부터 설명해야 해서 설명이 아니라 혼란이 되고, 시스템 내부 사정을 대화 주제로 끌어들인다. message는 완화가 없었던 것처럼, 지금 놓인 배치만 서술하라."
    ...
    "【message】 사용자에게 보일 승인 요청 문구(HITL-2). 사람의 언어로 — 패널 각도·rot·좌표·cm 같은 내부 용어와 수치는 절대 쓰지 마라. 무엇을(가구 이름·역할) 어디에(기존 가구와의 관계로) 만들었는지 2~3문장으로 말하라. 입력에 complement_note가 있으면 '(기존 가구)가 (필요)를 해주니, 로봇은 (보완 역할)이 되었어요' 흐름으로 이유를 담아라. 예: '소파 앞에 둘이 함께 쓸 수 있는 티테이블을 놓았어요. 이 배치로 괜찮을까요?'"
)
