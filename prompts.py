...
INTENT_PROMPT = (
    ...
    "입력에 '방의 기존 가구(pre_existing_furniture)'가 주어지면 반드시 고려하라. 원칙은 '중복하지 말되, 반드시 기여하라'다. (1) 활동에 필요한 가구를 전부 떠올린 뒤, 방에 이미 있는 가구가 그 활동의 필요를 '실제로 충족'하는 것만 목록에서 빼라 — 판단 기준은 이름·카테고리 일치가 아니라 적합성이다. 기존 가구가 그 활동을 하기에 높이·크기·전용 면적 면에서 충분한지를 따져라.  카테고리만 겹칠 뿐 그 활동엔 부적합한 기존 가구는 '중복'이 아니다. (2) 그렇게 뺀 결과가 빈 목록이 되어서는 안 된다. 기존 가구가 활동의 중심을 실제로 충족해 뺐더라도, 로봇이 곁에서 그 활동을 더 낫게 만드는 보완 가구를 최소 하나 나열하라 — 거실에 책장이 이미 있는데 '책 읽고 싶어'면 책장을 또 만드는 게 아니라 책장 옆 독서 의자·독서대를. 기준은 '이 방에서 이 활동을 하는 사람에게 로봇이 무엇을 더해주면 가장 도움이 되는가'다. 단, 이 원칙은 활동을 지원하라는 발화에만 적용된다 — '다 치워줘'처럼 로봇 제거가 목적인 발화(remove)는 빈 목록이 올바르다."
    ...
    "【confirmation_message】 파악한 내용을 사용자에게 확인하는 한 문장. 형식을 고정하라: '(인원·활동 요약)하시는군요, (공간)에 (준비할 가구·역할과 개수)를 준비해드릴게요.' 항상 이 순서(상황 요약 → 어디에 무엇을 몇 개 준비할지)를 지키고, 패널 각도·좌표 같은 로봇 내부 용어는 쓰지 말고 사람이 쓰는 가구 이름과 개수로 표현하라. 입력의 새 발화에 '(확인 답변)'이 포함돼 있다면 직전에 되묻기를 거쳐 답을 받은 것이니 '네,'로 시작해 답변을 반영했음을 드러내라. 예: (되묻기 없이) '친구와 함께 공작 놀이를 하시는군요, 거실에 넓은 작업 테이블을 준비해드릴게요.' / (되묻기 후) '네, 두 분이 함께 식사하시는군요. 부엌에 식탁 옆 의자 2개를 준비해드릴게요.'"
)
...
INTENT_SCHEMA = {
    ...
    "properties": {
        ...
        "confirmation_message": {
            "type": "string",
            "description": "HITL-1(human-in-the-loop)용 의도 확인 발화 한 문장. 형식 고정: '(인원·활동 요약)하시는군요, (공간)에 (준비할 가구·역할과 개수)를 준비해드릴게요.' 로봇 내부 용어(패널 각도·좌표) 금지, 가구 이름과 개수로 표현. 되묻기(clarification) 답변을 반영하는 경우 '네,'로 시작",
        },
    },
    "required": [
